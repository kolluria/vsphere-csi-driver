	...
	ginkgo.It("[csi-block-vanilla][csi-guest][csi-supervisor]"+
		...
		if wcpVsanDirectCluster && supervisorCluster {
			...
			for _, p := range podList.Items {
				pod, err = client.CoreV1().Pods(namespace).Get(ctx, p.Name, metav1.GetOptions{})
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				err = fpod.WaitTimeoutForPodRunningInNamespace(ctx,
					client, pod.Name, namespace, pollTimeout*2)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				pods = append(pods, pod)
			}
		}
		...
		pvs, err := fpv.WaitForPVClaimBoundPhase(ctx, client,
			pvcs, framework.ClaimProvisionTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("[csi-block-vanilla][csi-guest][csi-supervisor]"+
		...
		if wcpVsanDirectCluster && supervisorCluster {
			...
			for _, p := range podList.Items {
				...
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				err = fpod.WaitTimeoutForPodRunningInNamespace(ctx,
					client, pod.Name, namespace, pollTimeout*3)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				pods = append(pods, pod)
			}
		}
