...
func deletePodsInParallel(ctx context.Context, client clientset.Interface, namespace string, pods []*v1.Pod, wg *sync.WaitGroup) {
	defer ginkgo.GinkgoRecover()
	defer wg.Done()
...
func createPvcInParallel(ctx context.Context, client clientset.Interface, namespace string, diskSize string, sc *storagev1.StorageClass,
...
func createPodsInParallel(client clientset.Interface, namespace string, pvclaims []*v1.PersistentVolumeClaim,
	...
	for i := 0; i < volumeOpsScale; i++ {
		pod := fpod.MakePod(namespace, nil, []*v1.PersistentVolumeClaim{pvclaims[i]}, admissionapi.LevelBaseline, execCommand)
...
func volumeLifecycleActions(ctx context.Context, client clientset.Interface, namespace string,
	...
	pod1, err := createPod(ctx,
		client, namespace, nil, []*v1.PersistentVolumeClaim{pvc1}, false, execCommand)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	err = fpv.DeletePersistentVolumeClaim(ctx,
		client, pvc1.Name, namespace)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
func scaleDownStsAndVerifyPodMetadata(ctx context.Context, client clientset.Interface,
	...
	if isScaleDownRequired {
		...
		_, scaledownErr := fss.Scale(ctx,
			client, statefulset, replicas)
		gomega.Expect(scaledownErr).NotTo(gomega.HaveOccurred())
	}

	fss.WaitForStatusReadyReplicas(ctx,
		client, statefulset, replicas)
	ssPodsAfterScaleDown := fss.GetPodList(ctx,
		client, statefulset)
...
func scaleUpStsAndVerifyPodMetadata(ctx context.Context, client clientset.Interface,
	...
	if verifyCnsVolumes {
		...
		for _, sspod := range ssPodsAfterScaleUp.Items {
			err := fpod.WaitTimeoutForPodReadyInNamespace(ctx, client,
				sspod.Name, statefulset.Namespace, pollTimeout)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
func deleteCsiPodInParallel(ctx context.Context, client clientset.Interface, pod *v1.Pod, namespace string, wg *sync.WaitGroup) {
	defer ginkgo.GinkgoRecover()
	defer wg.Done()
