...
var _ = ginkgo.Describe("[csi-guest] CnsNodeVmAttachment persistence", func() {
	...
	ginkgo.It("Verify CnsNodeVmAttachements crd and Pod is created after CSI controller comes up", func() {
		...
		ginkgo.By("Creating Storage Class and PVC")
		scParameters[svStorageClassName] = storagePolicyName
		sc, pvc, err = createPVCAndStorageClass(ctx, client,
	...
	ginkgo.It("Create a Pod mounted with multiple PVC", func() {
		...
		pod, err := createPod(ctx, client, namespace, nil,
			[]*v1.PersistentVolumeClaim{pvc}, false, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.It("Verify PVC is attached to Pods created in corresponding GC", func() {
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		pvclaim, err = createPVC(ctx, client,
			namespace, nil, "", storageclass, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		pvc, err := createPVC(ctx, clientNewGc,
			namespaceNewGC, nil, "", storageclassNewGC, "")
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		pod, err := createPod(ctx, clientNewGc,
			namespaceNewGC, nil, []*v1.PersistentVolumeClaim{pvcNew}, false, execCommand)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		pod1, err := createPod(ctx, client,
			namespace, nil, []*v1.PersistentVolumeClaim{pvclaim}, false, execCommand)
