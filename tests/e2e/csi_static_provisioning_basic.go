...
var _ = ginkgo.Describe("Basic Static Provisioning", func() {
	...
	testCleanUpUtil := func(ctx context.Context, restClientConfig *restclient.Config,
		...
		framework.ExpectNoError(fpv.DeletePersistentVolumeClaim(ctx, client, pvcName, namespace), "Failed to delete PVC", pvcName)
		pvc = nil

		ginkgo.By("Verify PV should be deleted automatically")
		framework.ExpectNoError(fpv.WaitForPersistentVolumeDeleted(ctx, client, pvName, poll, supervisorClusterOperationsTimeout))
		pv = nil
	...
	ginkgo.It("[csi-block-vanilla] [csi-block-vanilla-parallelized] Verify basic static provisioning "+
		...
		framework.ExpectNoError(fpv.WaitOnPVandPVC(ctx,
			client, f.Timeouts, namespace, pv, pvc))
	...
	ginkgo.It("[csi-guest] Static provisioning workflow II in guest "+
		...
		ginkgo.By("Deleting the PV Claim")
		framework.ExpectNoError(fpv.DeletePersistentVolumeClaim(ctx,
			client, pvc.Name, namespace),
			"Failed to delete PVC", pvc.Name)
		pvc = nil
