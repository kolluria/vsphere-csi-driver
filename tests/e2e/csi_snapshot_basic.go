...
var _ = ginkgo.Describe("Volume Snapshot Basic Test", func() {
	...
	*/
	...
	ginkgo.It("[block-vanilla-snapshot] Volume restore using snapshot a dynamic snapshot b "+
		...
		pod, err := createPod(ctx, client, namespace, nil,
			[]*v1.PersistentVolumeClaim{pvclaim2}, false,
			execRWXCommandPod1)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		pod2, err := createPod(ctx, client, namespace, nil,
			[]*v1.PersistentVolumeClaim{pvclaim3}, false,
			execRWXCommandPod1)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] Volume snapshot creation and restoration workflow "+
		...
		pvclaim, persistentVolumes := createPVCAndQueryVolumeInCNS(ctx,
			client, namespace, nil, "", diskSize, storageclass, true)
		volHandle := persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle)
		}
		...
		pod2, err := createPod(ctx, client, namespace,
			nodeSelector, []*v1.PersistentVolumeClaim{pvclaim2}, false,
			execCommand)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] Delete the namespace hosting the pvcs and "+
		...
		pvclaim, persistentVolumes := createPVCAndQueryVolumeInCNS(ctx, client, newNamespaceName, nil, "",
			diskSize, storageclass, true)
		volHandle := persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle)
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] Snapshot workflow for statefulsets", ginkgo.Label(p0, block,
		...
		*statefulset.Spec.Replicas = 2
		...
		err = fpv.DeletePersistentVolumeClaim(ctx, client, pvcToDelete.Name, namespace)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		newPV, err := fpv.WaitForPVClaimBoundPhase(ctx, client, []*v1.PersistentVolumeClaim{pvclaim3},
			framework.ClaimProvisionTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] Volume deletion with existing snapshots", ginkgo.Label(p0,
		...
		defer func() {
			err := fpv.DeletePersistentVolumeClaim(ctx, client, pvclaim.Name, namespace)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			err = fpv.DeletePersistentVolume(ctx, client, persistentvolumes[0].Name)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			err = e2eVSphere.waitForCNSVolumeToBeDeleted(volHandle)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}()
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] [tkg-snapshot] Create a pre-provisioned snapshot using "+
		...
		pvclaim, persistentVolumes := createPVCAndQueryVolumeInCNS(ctx, client, namespace,
	...
	*/
	ginkgo.It("[block-vanilla-snapshot] VC password reset during snapshot creation", ginkgo.Label(p1, block,
		...
		ginkgo.By("Reverting the password change")
		err = invokeVCenterChangePassword(ctx, username,
			nimbusGeneratedVcPwd, originalPassword, vcAddress, false, clientIndex)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		for i := 0; i < snapshotOpsScale; i++ {
			...
			if i == 1 {
				...
				if vanillaCluster {
					...
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					err = fpod.WaitForPodsRunningReady(ctx, c, csiSystemNamespace, int(csipods.Size()),
	...
	*/
	ginkgo.It("[block-vanilla-snapshot][tkg-snapshot] Scale-up creation of snapshots across multiple "+
		...
		for i := 0; i < volumeOpsScale; i++ {
			framework.Logf("Creating pvc%v", i)
			pvclaims[i], err = createPVC(ctx, client, namespace, nil, "",
				storageclass, v1.ReadWriteOnce)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Verify Snapshot creation should fail on supervisor cluster", ginkgo.Label(p0,
		...
		pvclaim, err := fpv.CreatePVC(ctx, svcClient, svcNamespace,
			getPersistentVolumeClaimSpecWithStorageClass(svcNamespace, diskSize, storageclass, nil,
				v1.ReadWriteOnce))
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		ginkgo.By("Expect claim to provision volume successfully")
		persistentvolumes, err := fpv.WaitForPVClaimBoundPhase(ctx, svcClient,
			[]*v1.PersistentVolumeClaim{pvclaim}, framework.ClaimProvisionTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to provision volume")
		volHandle := persistentvolumes[0].Spec.CSI.VolumeHandle
		gomega.Expect(volHandle).NotTo(gomega.BeEmpty())
		defer func() {
			err = fpv.DeletePersistentVolumeClaim(ctx, svcClient, pvclaim.Name, pvclaim.Namespace)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			err = e2eVSphere.waitForCNSVolumeToBeDeleted(volHandle)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}()
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Create restore volume snapshot in consistent order", ginkgo.Label(p0, snapshot,
		...
		pvcSpec := getPersistentVolumeClaimSpecWithDatasource(namespace, diskSize, storageclass, nil,
			v1.ReadWriteOnce, volumeSnapshot.Name, snapshotapigroup)
		pvclaim1, err := fpv.CreatePVC(ctx, client, namespace, pvcSpec)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		persistentvolume1, err := fpv.WaitForPVClaimBoundPhase(ctx, client, []*v1.PersistentVolumeClaim{pvclaim1},
			framework.ClaimProvisionTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	...
	ginkgo.It("[tkg-snapshot] Create dynamic snapshot in GC1 and referring it create pre-provisoned snapshot "+
		...
		pvclaim, persistentVolumes := createPVCAndQueryVolumeInCNS(ctx, client, namespace1.Name, nil, "",
			diskSize, storageclass, true)
		volHandle := persistentVolumes[0].Spec.CSI.VolumeHandle
		if guestCluster {
			volHandle = getVolumeIDFromSupervisorCluster(volHandle)
		...
		volumeSnapshot, snapshotContent, snapshotCreated,
			snapshotContentCreated, dynamicSnapshotId, err := createDynamicVolumeSnapshot(ctx, namespace1.Name, snapc,
			volumeSnapshotClass, pvclaim, volHandle, diskSize, true)
		defer func() {
			if snapshotCreated {
				framework.Logf("Deleting volume snapshot")
