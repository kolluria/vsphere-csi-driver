...
var _ = ginkgo.Describe("Prevent duplicate cluster ID", func() {
	...
	*/
	ginkgo.It("[csi-config-secret-block][csi-config-secret-file] Generate cluster id and then set cluster id "+
		...
		csipods, err = client.CoreV1().Pods(csiNamespace).List(ctx, metav1.ListOptions{})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = fpod.WaitForPodsRunningReady(ctx, client, csiNamespace, int(csipods.Size()), pollTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	*/
	ginkgo.It("[csi-config-secret-block][csi-config-secret-file] Create vsphere config secret with cluster "+
		"id value set with special characters", func() {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
	...
	*/
	ginkgo.It("[csi-config-secret-block][csi-config-secret-file] Delete CSI"+
		...
		err = fpod.WaitForPodsRunningReady(ctx, client, csiSystemNamespace, num_csi_pods,
			pollTimeout)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
